        ...
        while (true) {
            ...
            switch (gamePhase) {
                ...
                case "RELEASE":
                    ...
                    int myAvailableBonus = myCardsInHand[BONUS_CARD];
                    int possibleTechnicalDebts = myAvailableBonus;
                    for (Application application : applications) {
                        int missing = 0;
                        for (int i=0;i<ZONES_COUNT;++i) {
                            missing += Math.max(0, application.resources[i] - 2*myCardsInHand[i]);
                            possibleTechnicalDebts += 2*myCardsInHand[i];
                        }
                        System.err.println(String.format("Obj %d needs %d resources. I have %d bonus", application.id, missing, myAvailableBonus));
                        if (missing <= myAvailableBonus) {
                            bestApplication = application.id;
                            break;
                        } else if (myScore<5 && missing<= possibleTechnicalDebts && missing<3) {
                            bestApplication = application.id;
                            break;
                        }
                    }
